...
## Ph6 - Dry-run 模拟盘 ✅（代码就绪，待实际运行）

- [x] T6.1 dry-run 启动脚本 + 启动前安全检查（precheck_dryrun.py）
- [x] T6.2 DRYRUN_VALIDATION.md 模板就绪
- [ ] 连续运行 1-2 周且无重大偏离（待用户在 VPS 执行）

## Ph7 - 小资金实盘前安全门禁 ✅（代码就绪，待用户确认）

- [x] T7.1 LIVE_SAFETY_CHECKLIST 全部检查项定义完成（8 大类 30+ 项）
- [x] T7.2 live_guard 测试通过（15 测试），缺任一环境变量拒绝启动
- [ ] 用户明确书面确认进入小资金实盘（待用户确认）

## Ph8 - 部署与运维 ✅

- [x] T8.1 Docker healthcheck + restart 策略 + 日志轮转 + FastAPI 容器
- [x] T8.2 VPS 部署说明 docs/DEPLOYMENT.md（8 章完整指南）
